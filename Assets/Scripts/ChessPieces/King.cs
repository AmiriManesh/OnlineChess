...
public class King : Chesspiece
{
    ...
    public override SpecialMove GetSpecialMove(ref Chesspiece[,] board, ref List<Vector2Int[]> moveList, ref List<Vector2Int> availableMoves)
    {
        ...
        var _kingMove = moveList.Find(m => m[0].x == 4 && m[0].y == ((team == 0)? 0 : 7));
        var _leftRook = moveList.Find(m => m[0].x == 0 && m[0].y == ((team == 0)? 0 : 7));
        var _rightRook = moveList.Find(m => m[0].x == 7 && m[0].y == ((team == 0)? 0 : 7));

        if(_kingMove == null && currentX == 4)
        {
            //White team
            if(team == 0)
            {
